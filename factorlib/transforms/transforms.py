...
class VolumeWeightedMomentum:
    def __init__(self, window=30, prices_col=None, volume_col=None):
        self.window = window
        self.prices_column = prices_col
        self.volume_column = volume_col

    def transform(self, data):
        data = data.swaplevel(0, 1, axis=1)
        data[self.prices_col] = data[self.prices_col].pct_change()
        momentum = pd.DataFrame()
        momentum['v_weight_momentum'] = data[self.prices_col] * data[self.volume_col]
        momentum = momentum.rolling(self.window).sum()
        momentum = momentum.swaplevel(0, 1, axis=1)
        momentum = _rename_columns_after_transform(momentum, transform='vol_weighted_mom', attribute=str(self.window))
        return momentum
